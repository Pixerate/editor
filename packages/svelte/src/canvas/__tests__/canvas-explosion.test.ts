...
describe("Spatial Displacement Solvers", () => {
  ...
  it("computes bounding box for a set of nodes", () => {
    const nodes = [
      createNode("a", 10, 20, 100, 40),
      createNode("b", 50, 100, 200, 80),
    ];
  ...
  it("calculates Dagre reflow displacement anchoring origin node position", () => {
    ...
    const edges: CanvasEdge[] = [
      { id: "e1", source: "origin", target: "downstream" },
    ];

    const children = [
      createNode("c1", 0, 0, 120, 60),
      createNode("c2", 0, 0, 120, 60),
    ];
...
describe("createCanvasExplosion Rune Lifecycle & Reversibility", () => {
  ...
  it("explodes node, connects children, shifts downstream nodes, and collapses reversibly", () => {
    ...
    expect(
      graph.edges.some((e) => e.source === "origin" && e.target === "c1"),
    ).toBe(true);
    expect(
      graph.edges.some((e) => e.source === "origin" && e.target === "c2"),
    ).toBe(true);
  ...
  it("toggles explosion with toggleExplode", () => {
    ...
    explosion.toggleExplode(
      "origin",
      { childNodes: [child], duration: 0 },
      { duration: 0 },
    );
    expect(explosion.isExploded("origin")).toBe(false);
  });
  ...
  it("animates opacity for child nodes and connecting edges while preserving full opacity of displaced sibling nodes", () => {
    ...
    try {
      ...
      let graphChild = graph.nodes.find((n) => n.id === "c1")!;
      let graphEdge = graph.edges.find(
        (e) => e.source === "origin" && e.target === "c1",
      )!;
